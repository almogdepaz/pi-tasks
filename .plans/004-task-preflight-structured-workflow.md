...
- 2026-07-24: hardened delegation skill/docs for token-efficient task boundaries: compact action prompts, required non-trivial workflow metadata, context refs over pasted plans/docs/diffs, root-cause batching, bounded structured results, preflight requirements, no terminal completion inference, parent follow-up via `onCompletePrompt`, fail-fast target setup, and no cross-repo filesystem-store delegation or symlink workaround.
